...
namespace KhachSan.GUI
{
    ...
    {
        List<Room_DTO> rooms = Room_DAO.ROOM();
        public Room_GUI()
        {
            InitializeComponent();
            this.StartPosition = FormStartPosition.CenterScreen;
            comboBox1.Enabled = false;
            this.load_DATA_ROOM(rooms);
        ...
        private void load_DATA_ROOM(List<Room_DTO> rooms)
        {
            this.define_Column_Table();
            TableKhachSan.Rows.Clear();
            if (rooms != null)
            {
                foreach (Room_DTO room in rooms)
                {
                    TableKhachSan.Rows.Add(new string[]
                    {
                        room.getMaPhong(),
                        room.getTenPhong(),
                        room.getGiaPhong().ToString(),
                        room.getPhiDatPhong().ToString(),
                        room.getTinhTrang() == 0 ? "Phòng trống" : "Được đặt"
                    });
                }
            }
        }
